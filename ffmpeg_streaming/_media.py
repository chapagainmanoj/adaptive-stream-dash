    ...
    def output(
        ...
        **options,
        ...
        if output is None and clouds is None:
            ...
            self.output_ = os.path.join(
                tempfile.mkdtemp(prefix="ffmpeg_streaming_"), clouds.filename
            )
        else:
    ...
    def run(self, ffmpeg_bin, monitor: callable = None, **options):
        ...
        if async_run:
            ...
            if loop and loop.is_running():
                task = loop.create_task(self.async_run(ffmpeg_bin, monitor, **options))
